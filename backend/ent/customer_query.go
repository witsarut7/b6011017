...
func (cq *CustomerQuery) QueryPayment() *PaymentQuery {
...
func (cq *CustomerQuery) WithPayment(opts ...func(*PaymentQuery)) *CustomerQuery {
	query := &PaymentQuery{config: cq.config}
	for _, opt := range opts {
		opt(query)
	}
	cq.withPayment = query
	return cq
}
...
func (cq *CustomerQuery) sqlAll(ctx context.Context) ([]*Customer, error) {
	...
	if query := cq.withPayment; query != nil {
		...
		query.Where(predicate.Payment(func(s *sql.Selector) {
			s.Where(sql.InValues(customer.PaymentColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.customer_payment
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "customer_payment" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "customer_payment" returned %v for node %v`, *fk, n.ID)
			}
